...
setwd("~/Documents/Virginia Tech/Project_4_VCU_postmortem_SCZ/ChIP-seq_differential/promoter_count/cofactors/")
factors = read.csv('promoter_SCZ_Control_neuron.csv')
...
###############################SCZ vs. Control###############################################################
setwd("~/Documents/Virginia Tech/Project_4_VCU_postmortem_SCZ/ChIP-seq_differential/promoter_count/count files/")
raw_def = read.csv('SCZ_Control_neuron_promoter_count.csv')
...
write.csv(all_matrix,'All_SCZ_Control_neuron_promoter.csv')
write.csv(new_DEG_matrix,'Differential_SCZ_Control_neuron.csv')


##########################untrt_SCZ vs. Control_1####################
setwd("~/Documents/Virginia Tech/Project_4_VCU_postmortem_SCZ/ChIP-seq_differential/promoter_count/count files/")
raw_def = read.csv('untrt_SCZ_Control_1_neuron_promoter_count.csv')
rownames(raw_def) = raw_def[,4]
def = raw_def[,5:ncol(raw_def)]
peak_info = raw_def[,1:4]
meta = factors_untrt
...
# t_group_untrt = t(group_untrt)
# umap_results <- umap(t_group_untrt,n_neighbors=20,min_dist=0.1,n_components = 6)
# umaps = data.frame(umap_results[["layout"]])
...
dds=DESeqDataSetFromMatrix(countData = group_untrt,
                           colData = colData,
                           design = ~ condition + age + FRiP + gender + NSC + RSC + PBC + peaks)
#tmp = model.matrix(~group_list + gender + Age + PMI, colData)
...
all_matrix = data.frame(DEG_glia,all_glia)
write.csv(all_matrix,'All_untrt_SCZ_Control_1_neuron_promoter.csv')
write.csv(new_DEG_matrix,'Differential_untrt_SCZ_Control_1_neuron.csv')


##########################trt_SCZ vs. Control_2####################
setwd("~/Documents/Virginia Tech/Project_4_VCU_postmortem_SCZ/ChIP-seq_differential/promoter_count/count files/")
raw_def = read.csv('trt_SCZ_Control_2_neuron_promoter_count.csv')
rownames(raw_def) = raw_def[,4]
def = raw_def[,5:ncol(raw_def)]
peak_info = raw_def[,1:4]
meta = factors_trt
...
dds=DESeqDataSetFromMatrix(countData = group_trt,
                           colData = colData,
                           design = ~ condition + FRiP + gender + NSC + unique + PBC + peaks + RSC)
#tmp = model.matrix(~group_list + gender + Age + PMI, colData)
...
all_matrix = data.frame(DEG_glia,all_glia)
write.csv(all_matrix,'All_trt_SCZ_Control_2_neuron_promoter.csv')
...
options(stringsAsFactors = F)
setwd("~/Documents/Virginia Tech/Project_4_VCU_postmortem_SCZ/ChIP-seq_differential/promoter_count/cofactors/")
factors = read.csv('promoter_SCZ_Control_glia.csv')
row.names(factors) = factors[,1]
factors = factors[,-1]
factors$totalreads = round((factors$totalreads/1000000),2)
factors$uniquereads = round((factors$uniquereads/1000000),2)
...
###############################SCZ vs. Control###############################################################
setwd("~/Documents/Virginia Tech/Project_4_VCU_postmortem_SCZ/ChIP-seq_differential/promoter_count/count files/")
raw_def = read.csv('SCZ_Control_glia_promoter_count.csv')
rownames(raw_def) = raw_def[,4]
def = raw_def[,5:ncol(raw_def)]
peak_info = raw_def[,1:4]
meta = factors_all
...
dds=DESeqDataSetFromMatrix(countData = group_all,
                           colData = colData,
                           design = ~ condition + age + unique + NSC + RSC)
#tmp = model.matrix(~group_list + gender + Age + PMI, colData)
...
all_glia = group_all[rownames(DEG_glia),]
all_matrix = data.frame(DEG_glia,all_glia)
write.csv(all_matrix,'All_SCZ_Control_glia_promoter.csv')
write.csv(new_DEG_matrix,'Differential_SCZ_Control_glia.csv')


##########################untrt_SCZ vs. Control_1####################
setwd("~/Documents/Virginia Tech/Project_4_VCU_postmortem_SCZ/ChIP-seq_differential/promoter_count/count files/")
raw_def = read.csv('untrt_SCZ_Control_1_glia_promoter_count.csv')
rownames(raw_def) = raw_def[,4]
def = raw_def[,5:ncol(raw_def)]
peak_info = raw_def[,1:4]
meta = factors_untrt
...
dds=DESeqDataSetFromMatrix(countData = group_untrt,
                           colData = colData,
                           design = ~ condition + age + PMI + RSC + unique)
#tmp = model.matrix(~group_list + gender + Age + PMI, colData)
...
DEG_glia=na.omit(DEG_glia)
nrDEG_glia=DEG_glia[which(DEG_glia$padj<0.05 & abs(DEG_glia$log2FoldChange) > 0.26),]
# nrDEG_glia=DEG_glia[which(DEG_glia$padj<0.05 & abs(DEG_glia$log2FoldChange) > 0),]
choose_gene_neuron=rownames(nrDEG_glia)
choose_matrix=group_untrt[choose_gene_neuron,]
DEG_matrix = data.frame(nrDEG_glia,choose_matrix)
up = nrDEG_glia[which(nrDEG_glia$log2FoldChange < 0),]
...
all_glia = group_untrt[rownames(DEG_glia),]
all_matrix = data.frame(DEG_glia,all_glia)
write.csv(all_matrix,'All_untrt_SCZ_Control_1_glia_promoter.csv')
write.csv(new_DEG_matrix,'Differential_untrt_SCZ_Control_1_glia.csv')


##########################trt_SCZ vs. Control_2####################
setwd("~/Documents/Virginia Tech/Project_4_VCU_postmortem_SCZ/ChIP-seq_differential/promoter_count/count files/")
raw_def = read.csv('trt_SCZ_Control_2_glia_promoter_count.csv')
rownames(raw_def) = raw_def[,4]
def = raw_def[,5:ncol(raw_def)]
peak_info = raw_def[,1:4]
meta = factors_trt
...
dds=DESeqDataSetFromMatrix(countData = group_trt,
                           colData = colData,
                           design = ~ condition + age + FRiP + gender + NSC + RSC + total + unique)
#tmp = model.matrix(~group_list + gender + Age + PMI, colData)
...
all_glia = group_trt[rownames(DEG_glia),]
all_matrix = data.frame(DEG_glia,all_glia)
write.csv(all_matrix,'All_trt_SCZ_Control_2_glia_promoter.csv')
